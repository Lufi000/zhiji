import Foundation

// MARK: - 月运势计算器

/// 月运势计算器
/// 基于八字命理计算每月运势
struct MonthlyFortuneCalculator {

    // MARK: - 公开方法

    /// 生成指定月数的月运势列表
    /// - Parameters:
    ///   - riGan: 日主天干
    ///   - bazi: 八字命盘
    ///   - monthsCount: 生成月数（默认12个月）
    ///   - startDate: 起始日期（默认当前日期）
    /// - Returns: 月运势数组
    static func generateMonthlyFortunes(
        riGan: String,
        bazi: Bazi,
        monthsCount: Int = 12,
        startDate: Date = Date()
    ) -> [MonthlyFortune] {
        var fortunes: [MonthlyFortune] = []
        let calendar = Calendar.current

        let startYear = calendar.component(.year, from: startDate)
        let startMonth = calendar.component(.month, from: startDate)

        for i in 0..<monthsCount {
            let totalMonths = startMonth - 1 + i
            let year = startYear + totalMonths / 12
            let month = totalMonths % 12 + 1

            let fortune = calculateMonthlyFortune(
                riGan: riGan,
                bazi: bazi,
                year: year,
                month: month
            )
            fortunes.append(fortune)
        }

        return fortunes
    }

    /// 获取当前月的运势
    static func getCurrentMonthFortune(riGan: String, bazi: Bazi) -> MonthlyFortune {
        let calendar = Calendar.current
        let now = Date()
        let year = calendar.component(.year, from: now)
        let month = calendar.component(.month, from: now)
        return calculateMonthlyFortune(riGan: riGan, bazi: bazi, year: year, month: month)
    }

    // MARK: - 私有方法

    /// 计算单月运势
    private static func calculateMonthlyFortune(
        riGan: String,
        bazi: Bazi,
        year: Int,
        month: Int
    ) -> MonthlyFortune {
        // 获取月柱干支
        let monthPillar = getMonthPillar(year: year, month: month)

        // 计算十神
        let ganShiShen = getShiShen(riGan: riGan, targetGan: monthPillar.gan)
        let zhiShiShen = getZhiShiShen(riGan: riGan, zhi: monthPillar.zhi)

        // 计算运势评分
        let (score, level) = calculateFortuneScore(
            riGan: riGan,
            bazi: bazi,
            monthGan: monthPillar.gan,
            monthZhi: monthPillar.zhi
        )

        // 生成关键词和建议
        let keywords = generateKeywords(ganShiShen: ganShiShen, zhiShiShen: zhiShiShen, level: level)
        let advice = generateAdvice(ganShiShen: ganShiShen, zhiShiShen: zhiShiShen, level: level)

        return MonthlyFortune(
            year: year,
            month: month,
            gan: monthPillar.gan,
            zhi: monthPillar.zhi,
            ganShiShen: ganShiShen,
            zhiShiShen: zhiShiShen,
            fortuneScore: score,
            fortuneLevel: level,
            keywords: keywords,
            advice: advice
        )
    }

    /// 计算月柱干支
    /// 月柱的计算基于节气，这里使用简化算法
    private static func getMonthPillar(year: Int, month: Int) -> (gan: String, zhi: String) {
        // 月支固定：正月寅，二月卯...
        let monthZhiIndex = (month + 1) % 12  // 正月=寅(2)
        let zhi = BaziConstants.diZhi[monthZhiIndex]

        // 月干根据年干推算
        // 年干决定正月天干：甲己年丙寅月，乙庚年戊寅月，丙辛年庚寅月，丁壬年壬寅月，戊癸年甲寅月
        let yearGanIndex = (year - 4) % 10
        let baseGanIndex: Int
        switch yearGanIndex {
        case 0, 5:  // 甲、己年
            baseGanIndex = 2  // 丙
        case 1, 6:  // 乙、庚年
            baseGanIndex = 4  // 戊
        case 2, 7:  // 丙、辛年
            baseGanIndex = 6  // 庚
        case 3, 8:  // 丁、壬年
            baseGanIndex = 8  // 壬
        case 4, 9:  // 戊、癸年
            baseGanIndex = 0  // 甲
        default:
            baseGanIndex = 0
        }

        let ganIndex = (baseGanIndex + month - 1) % 10
        let gan = BaziConstants.tianGan[ganIndex]

        return (gan, zhi)
    }

    /// 计算运势评分和等级
    private static func calculateFortuneScore(
        riGan: String,
        bazi: Bazi,
        monthGan: String,
        monthZhi: String
    ) -> (Int, FortuneLevel) {
        var score = 50  // 基础分

        // 获取日主五行
        guard let riWuXing = BaziConstants.wuXing[riGan],
              let monthGanWuXing = BaziConstants.wuXing[monthGan],
              let monthZhiWuXing = BaziConstants.wuXing[monthZhi] else {
            return (50, .neutral)
        }

        // 1. 天干关系评分 (+/- 15分)
        score += evaluateGanRelation(riWuXing: riWuXing, targetWuXing: monthGanWuXing)

        // 2. 地支关系评分 (+/- 15分)
        score += evaluateZhiRelation(riWuXing: riWuXing, targetWuXing: monthZhiWuXing)

        // 3. 与八字的合冲关系 (+/- 20分)
        score += evaluateBaziRelation(bazi: bazi, monthGan: monthGan, monthZhi: monthZhi)

        // 限制分数范围
        score = max(10, min(95, score))

        // 确定等级
        let level: FortuneLevel
        switch score {
        case 80...: level = .excellent
        case 65..<80: level = .good
        case 45..<65: level = .neutral
        case 30..<45: level = .caution
        default: level = .poor
        }

        return (score, level)
    }

    /// 评估天干五行关系
    private static func evaluateGanRelation(riWuXing: String, targetWuXing: String) -> Int {
        // 生我者（正印、偏印）+15
        if BaziConstants.wuXingSheng[targetWuXing] == riWuXing {
            return 15
        }
        // 同我者（比肩、劫财）+10
        if riWuXing == targetWuXing {
            return 10
        }
        // 我生者（食神、伤官）+5
        if BaziConstants.wuXingSheng[riWuXing] == targetWuXing {
            return 5
        }
        // 我克者（正财、偏财）-5
        if BaziConstants.wuXingKe[riWuXing] == targetWuXing {
            return -5
        }
        // 克我者（正官、七杀）-15
        if BaziConstants.wuXingKe[targetWuXing] == riWuXing {
            return -15
        }
        return 0
    }

    /// 评估地支五行关系
    private static func evaluateZhiRelation(riWuXing: String, targetWuXing: String) -> Int {
        if BaziConstants.wuXingSheng[targetWuXing] == riWuXing {
            return 12
        }
        if riWuXing == targetWuXing {
            return 8
        }
        if BaziConstants.wuXingSheng[riWuXing] == targetWuXing {
            return 4
        }
        if BaziConstants.wuXingKe[riWuXing] == targetWuXing {
            return -4
        }
        if BaziConstants.wuXingKe[targetWuXing] == riWuXing {
            return -12
        }
        return 0
    }

    /// 评估与八字的合冲关系
    private static func evaluateBaziRelation(bazi: Bazi, monthGan: String, monthZhi: String) -> Int {
        var score = 0
        let baziZhis = [bazi.year.zhi, bazi.month.zhi, bazi.day.zhi, bazi.hour.zhi]
        let baziGans = [bazi.year.gan, bazi.month.gan, bazi.day.gan, bazi.hour.gan]

        // 检查地支六合 (+8)
        for liuHe in BaziConstants.diZhiLiuHe {
            for baziZhi in baziZhis {
                if (liuHe.0 == monthZhi && liuHe.1 == baziZhi) ||
                   (liuHe.1 == monthZhi && liuHe.0 == baziZhi) {
                    score += 8
                }
            }
        }

        // 检查天干五合 (+6)
        for tianGanHe in BaziConstants.tianGanHe {
            for baziGan in baziGans {
                if (tianGanHe.0 == monthGan && tianGanHe.1 == baziGan) ||
                   (tianGanHe.1 == monthGan && tianGanHe.0 == baziGan) {
                    score += 6
                }
            }
        }

        // 检查地支六冲 (-12)
        for chong in BaziConstants.diZhiChong {
            for baziZhi in baziZhis {
                if (chong.0 == monthZhi && chong.1 == baziZhi) ||
                   (chong.1 == monthZhi && chong.0 == baziZhi) {
                    score -= 12
                }
            }
        }

        // 检查地支相刑 (-8)
        for xing in BaziConstants.diZhiXing {
            for baziZhi in baziZhis {
                if (xing.0 == monthZhi && xing.1 == baziZhi) ||
                   (xing.1 == monthZhi && xing.0 == baziZhi) {
                    score -= 8
                }
            }
        }

        // 检查地支相害 (-6)
        for hai in BaziConstants.diZhiHai {
            for baziZhi in baziZhis {
                if (hai.0 == monthZhi && hai.1 == baziZhi) ||
                   (hai.1 == monthZhi && hai.0 == baziZhi) {
                    score -= 6
                }
            }
        }

        return max(-20, min(20, score))
    }

    /// 生成关键词
    private static func generateKeywords(ganShiShen: String, zhiShiShen: String, level: FortuneLevel) -> [String] {
        var keywords: [String] = []

        // 根据天干十神添加关键词
        switch ganShiShen {
        case "比肩": keywords.append("合作")
        case "劫财": keywords.append("竞争")
        case "食神": keywords.append("创意")
        case "伤官": keywords.append("表达")
        case "偏财": keywords.append("机遇")
        case "正财": keywords.append("收获")
        case "七杀": keywords.append("挑战")
        case "正官": keywords.append("责任")
        case "偏印": keywords.append("学习")
        case "正印": keywords.append("贵人")
        default: break
        }

        // 根据地支十神添加关键词
        switch zhiShiShen {
        case "比肩": keywords.append("人脉")
        case "劫财": keywords.append("变动")
        case "食神": keywords.append("享受")
        case "伤官": keywords.append("突破")
        case "偏财": keywords.append("投资")
        case "正财": keywords.append("稳定")
        case "七杀": keywords.append("压力")
        case "正官": keywords.append("规范")
        case "偏印": keywords.append("思考")
        case "正印": keywords.append("支持")
        default: break
        }

        // 根据运势等级添加关键词
        switch level {
        case .excellent: keywords.append("顺遂")
        case .good: keywords.append("进展")
        case .neutral: keywords.append("平稳")
        case .caution: keywords.append("谨慎")
        case .poor: keywords.append("蛰伏")
        }

        return Array(keywords.prefix(3))
    }

    /// 生成建议
    private static func generateAdvice(ganShiShen: String, zhiShiShen: String, level: FortuneLevel) -> String {
        let ganAdvice = getShiShenAdvice(ganShiShen)
        let levelAdvice = getLevelAdvice(level)
        return "\(ganAdvice)\(levelAdvice)"
    }

    /// 获取十神相关建议
    private static func getShiShenAdvice(_ shiShen: String) -> String {
        switch shiShen {
        case "比肩": return "本月适合团队协作，与同事朋友合作能事半功倍。"
        case "劫财": return "本月注意竞争关系，保护好自己的资源和机会。"
        case "食神": return "本月灵感丰富，适合创作和休闲放松。"
        case "伤官": return "本月表达欲强，但需注意言辞分寸。"
        case "偏财": return "本月有意外收获的机会，可留意投资信息。"
        case "正财": return "本月财运稳健，适合理财规划。"
        case "七杀": return "本月压力较大，需保持冷静应对挑战。"
        case "正官": return "本月工作运佳，适合处理正事和提升。"
        case "偏印": return "本月适合学习新技能，思考人生方向。"
        case "正印": return "本月贵人运好，多向长辈前辈请教。"
        default: return "本月保持平常心，稳步前行。"
        }
    }

    /// 获取等级相关建议
    private static func getLevelAdvice(_ level: FortuneLevel) -> String {
        switch level {
        case .excellent: return "整体运势大吉，可大胆行动。"
        case .good: return "运势顺畅，把握机会。"
        case .neutral: return "运势平稳，宜守不宜攻。"
        case .caution: return "需多加谨慎，避免冲动决策。"
        case .poor: return "宜低调行事，蓄势待发。"
        }
    }
}
